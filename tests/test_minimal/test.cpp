...
void problem_2() {
    ...
    std::vector<NormalizedSystem> sys_x    = std::vector<NormalizedSystem>(4);
    std::vector<NormalizedSystem> sys_xy   = std::vector<NormalizedSystem>(4);
    std::vector<NormalizedSystem> fa_y_sys = std::vector<NormalizedSystem>(4);
    std::vector<Ctc*>             x_ctc_id = std::vector<Ctc*>(4);          
    std::vector<Ctc*>             xy_ctc   = std::vector<Ctc*>(4);          
    std::vector<Ctc*>             fa_y_ctc = std::vector<Ctc*>(4);          

    for (int i = 0 ; i<4 ; i++) {
        sys_x.push_back(     NormalizedSystem(fac_x));
        sys_xy.push_back(    NormalizedSystem(fac_xy));
        fa_y_sys.push_back(  NormalizedSystem(fac_fa_y_sys));
